function get_Same_Hemisphere_Channels_Analysis_PrePost(paths, selDetector, analysisPerZonesTablePath, thList, features, normOptions, freqBandConnList, patsSelList, outcomeTh, connTh, eiTh)
    analysisType = {'Same_Hemisphere_Channels'};    
    analysis = {};
    threshold = [];
    nrPats = [];
    nrChanns = [];
        ...
        for ni = 1:length(normOptions)
            for fi = 1:length(features)
                for psi = 1:length(patsSelList)
                    ...
                    tableDelIdxs = not(sameRFTC_HemisphereChannsPerPat(groupTablePre.patName, groupTablePre.rftcVals, groupTablePre.hemisphere));
