function get_Opposite_BrainLobe_Channels_Analysis_PrePost(paths, selDetector, analysisPerZonesTablePath, thList, features, normOptions, freqBandConnList, patsSelList, outcomeTh, connTh, eiTh)
    analysisType = {'Opposite_BrainLobe_Channels'};
    analysis = {};
    threshold = [];
    nrPats = [];
    nrChanns = [];
    ...
    for th = thList
        for ni = 1:length(normOptions)
            for fi = 1:length(features)
                for psi = 1:length(patsSelList)
                    ...
                    tableDelIdxs = (sameRFTC_BrainLobeChannsPerPat(groupTablePre));
        ...
        analysisPerZonesTableFN = strcat(analysisPerZonesTablePath, 'AnalysisPerBrainLobe_', newSelDetector,'.xls');
