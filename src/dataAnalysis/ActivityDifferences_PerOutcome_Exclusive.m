...
for fi = 1:length(features)
    ...
    for bmi = 1:length(biomarkersList)
        ...
        nrImprovedPats = length(unique(groupTablePre.patName));
        ...
        nrNonImprovedPats = length(unique(groupTablePre.patName));
    ...
    plotOutcomeDifferences_Barchart(predictTableDiff, predictTablePVal, params);
...
function [groupTablePre, groupTablePost] = readTables(paths, params)
    ...
    predPosPats = {'ALVj';'ARNj';'AUBa';'BROg';'CHRc';'CUSc';'ECKf';'LECj'};
    predPosPats = {'ALVj';'ARNj';'AUBa';'BROg';'CUSc';'LECl'};
    predPosPats = {'ALVj'; 'ARNj'; 'AUBa'; 'BROg'; 'CHRc'; 'CUSc'; 'ECKf'; 'LECl'; 'MARf'; 'SELc'};
    predPosPats = {'ALVj'; 'ARNj'; 'AUBa'; 'BROg'; 'CHRc'; 'CUSc'; 'ECKf'; 'LECj'}; %Cos
    predPosPats = {'ALVj'; 'ARNj'; 'AUBa'; 'BROg'; 'CUSc'; 'LECl'}; %Correlation
    predPosPats = {'ALVj'; 'ARNj'; 'AUBa'; 'BROg'; 'CHRc'; 'CUSc'; 'ECKf'; 'LECl'; 'MARf'; 'SELc'}; % Square Euclidean
    selPredictedPosPatsFlags = ismember(groupTablePost.patName, predPosPats);
    predictedPosPatNames = unique(groupTablePost.patName(selPredictedPosPatsFlags));

    %% Select outcomes
    if strcmp(params.patsSel, 'improvedPatients')
        tableDelIdxsIn = groupTablePre.outcome < params.outcomeTh;
        %tableDelIdxsIn = selPredictedPosPatsFlags;
        groupTablePre(tableDelIdxsIn, :) = [];
        groupTablePost(tableDelIdxsIn, :) = [];        
    elseif strcmp(params.patsSel, 'nonImprovedPatients')
        tableDelIdxsIn = groupTablePre.outcome >= params.outcomeTh;
        %tableDelIdxsIn = not(selPredictedPosPatsFlags);
        groupTablePre(tableDelIdxsIn, :) = [];
        groupTablePost(tableDelIdxsIn, :) = []; 
    end
end
