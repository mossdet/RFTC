...
%             end
            ...
            dataP = [allZones(fi,:); allZones(si,:)+1];
            ...
            spider_plot_R2019b(dataP,...
                ...
                'LineStyle', '-',...
                'LineWidth', 0.01,...
                'LineTransparency', [1 1],...
                'Marker', 'none',...
                'MarkerSize', 1,...
                'MarkerTransparency', [0.1 0.1],...
                'FillOption', {'on', 'off'},...
                'FillTransparency', [0 0],...
                'Color', [colors{1}; colors{3}]);
    
%             nrChanns = sum(dataP);
%             nrPats = length(unique(patNames(allZones(fi,:)')));
%             subtitleStr = {labels{fi}; strcat("(", num2str(nrPats), ", ", num2str(nrChanns), ")")};
%             subtitleStr = {labels{fi}; strcat("(", num2str(nrChanns), ")")};
...
%             title(subtitleStr, 'FontSize', 14);
            ...
            %[mcc,pVal] = corr(allZones(fi,:)', allZones(si,:)','Type','Spearman');
            %[mcc,pVal] = getChannelsOverlap(allZones(fi,:), allZones(si,:));
            mcc = round(mcc*100);
            text(0, 0, num2str(mcc), 'FontSize', 14, 'FontWeight','bold','HorizontalAlignment', 'center');
            %ht = text(0, -1.2, strcat("p<", num2str(pVal)), 'FontSize', 10, 'HorizontalAlignment', 'center');

            if fi == 1
                title(labels(si), 'FontSize', 14);
            end
            if si == 1
                ht = text(-1.5, 0, labels{fi}, 'FontSize', 14, 'FontWeight','bold','HorizontalAlignment', 'center');
                set(ht,'Rotation',90);
            end
        end
    end

    imageFN = strcat(figuresOutputPath, 'Zone_Zone_Overlap_MCC');
    saveas(gcf,imageFN);
    imageFN = strcat(imageFN, '.jpg');
    saveas(gcf,imageFN);
    close();
end
