function plotZone_ZoneCorrelationBars(params, groupTablePre)
    close all;
...
%     problemChildSel = ismember(patNames, 'LECl');
%     problemChildSel = not(rftcChanns) & not(rftcStructure_Channs) &problemChildSel;
%     problemTable = groupTablePre(problemChildSel, :);
    ...
    % define Labels
    labels = {'RFTC', 'highEI', 'highRFTC-Conn.', 'rftcStructure', 'rftcLobe', 'rftcHemisphere'};
    ...
    for si = 1:size(allZones,1)
        nexttile
        dataP = allZones(si,:);
        bar(dataP,'FaceColor',colors{si},'EdgeColor','none','BarWidth',1.5)
        xlim([1 length(dataP)])
        if si >= 5
            xlabel('Channels')
        end
        ...
        ylim([min(double(dataP)) max(double(dataP))]);
        yticks([0 1]);
        ...
        subtitleStr = {labels{si}; strcat(num2str(nrPats), " Patients"); strcat(num2str(nrChanns), "Channels")};
        title(subtitleStr, 'FontSize', 16, 'Units', 'normalized', 'Position', [0.5, 0.6, 0])
    end
    ...
    if(params.exlusive)
        boxchartFN = strcat(boxchartFN, '_Exclusive');
    end
