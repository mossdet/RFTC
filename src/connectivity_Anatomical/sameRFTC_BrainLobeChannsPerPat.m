function allPatsKeepIdxs = sameRFTC_BrainLobeChannsPerPat(groupTable)
    ...
    for pi = 1:length(patNames)
        ...
        %rftcLobes
        patKeepIdx = ismember(patLobeVals, rftcLobes);
        allPatsKeepIdxs(patSelIdx) = patKeepIdx;
        sameLobeNonRFTC = patKeepIdx;
        sameLobeNonRFTC = patKeepIdx & (not(logical(patRFTC_Vals)));
        if sum(sameLobeNonRFTC) == 0
            patName
            stopHere = 0;
        end
    end
