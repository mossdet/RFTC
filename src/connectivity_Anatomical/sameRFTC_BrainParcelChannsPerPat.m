function allPatsKeepIdxs = sameRFTC_BrainParcelChannsPerPat(groupTable)
    ...
    for pi = 1:length(patNames)
        ...
        patParcelValsA = patParcelVals;
        patParcelVals = getBrainParcels(patName, patChannels);
        
        %patParcelVals(strcmp(patParcelVals, 'N/A')) = [];
        patParcels = unique(patParcelVals);
        rftcParcels = unique(patParcelVals(logical(patRFTC_Vals)));
        rftcParcels(strcmp(rftcParcels, 'N/A')) = [];
        %rftcParcels
        patKeepIdx = ismember(patParcelVals, rftcParcels);
        allPatsKeepIdxs(patSelIdx) = patKeepIdx;
        if sum(patKeepIdx & not(patRFTC_Vals)) == 0
            patName
        end
    end
...
function channelParcelling = getBrainParcels(patName, channels)

    %channelParcelling = getEstimatedBrainParcels(patName, channels);
    channelParcelling = getGrenobleBrainParcels(patName, channels);
...
function channelParcelling = getGrenobleBrainParcels(patName, channels)
    channelParcelling = {};
